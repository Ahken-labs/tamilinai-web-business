...
function EditServiceAreasModal({
  ...
  return (
    <EditModal title={t("Edit_service_areas")} subtitle={t("Service_areas_description")} onClose={onClose} onSave={handleSave} saveText={t("Save_changes")}>
...
export default function EditServiceAreasSection() {
  const { t } = useLang();

  const [districts, setDistricts] = useState<string[]>(DUMMY_DATA.districts);
  const [editOpen, setEditOpen] = useState(false);

  const islandWide = districts.length === SRI_LANKA_DISTRICTS.length;
      ...
      <div className="max-w-[640px] mx-auto">
        ...
        {editOpen && (
          <EditServiceAreasModal
            ...
            onSave={(d) => { setDistricts(d); setEditOpen(false); }}
          />
        )}
      </div>
    </div>
